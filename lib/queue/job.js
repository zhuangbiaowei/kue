...
function getSearch() {
    ...
    reds.createClient = require('../redis')
        .createClient;
    ...
    return function(err, ids) {
        ...
        ids.forEach(function(id) {
            exports.get(id, function(err, job) {
                if (err) return fn(err);
                jobs[job.id] = job;
                --pending || fn(null, 'desc' == order ? map(jobs, ids)
                    .reverse() : map(jobs, ids));
            });
        });
    }
}
...
exports.range = function(from, to, order, fn) {
    redis.client()
        .zrange('q:jobs', from, to, get(fn, order));
};
...
exports.rangeByState = function(state, from, to, order, fn) {
    redis.client()
        .zrange('q:jobs:' + state, from, to, get(fn, order));
};
...
exports.rangeByType = function(type, state, from, to, order, fn) {
    redis.client()
        .zrange('q:jobs:' + type + ':' + state, from, to, get(fn, order));
};


exports.rangeByParent = function(parent_id, from, to, fn) {
    redis.client()
        .zrange('q:jobs:parent:' + parent_id, from, to, get(fn, 'desc'));
};
...
exports.get = function(id, fn) {
    ...
    client.hgetall('q:job:' + job.id, function(err, hash) {
        ...
        job.type = hash.type;
        job.phase = hash.phase;
        job.parent_id = hash.parent_id;
        ...
        job.return_value = hash.return_value;
        try {
...
function Job(type, data, parent_id) {
    this.type = type;
    this.phase = 'start';
    this.data = data || {};
...
Job.prototype.removeState = function(fn) {
    ...
    if (this.parent_id > 0) {
        client.zrem('q:jobs:parent:' + this.parent_id, this.id);
    }
    return this;
};
...
Job.prototype.state = function(state) {
    ...
    client.zadd('q:jobs:' + this.type + ':' + this.phase + ':' + state, this._priority, this.id);
    if (this.parent_id > 0) {
        client.zadd('q:jobs:parent:' + this.parent_id, this._priority, this.id);
    }
    // increase available jobs, used by Worker#getJob()
    if ('inactive' == state) {
        client.lpush('q:' + this.type + ':' + this.phase + ':jobs', 1);
    }
    return this;
};
...
Job.prototype.save = function(fn) {
    ...
    client.incr('q:ids', function(err, id) {
        ...
        self.set('phase', self.phase);
        self.set('parent_id', self.parent_id);
